...
function readContact(string $file, array $default): array {
...
function parseJsonField(string $value, string $label): array {
    $value = trim($value);
    if ($value === '') return [];
    $decoded = json_decode($value, true);
    if (!is_array($decoded)) throw new RuntimeException("$label valid JSON object yoki array bo‘lishi kerak.");
    return $decoded;
}

$data = readContact($file, $default);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $socialRaw = trim((string)($_POST['social'] ?? ''));
    ...
    try {
        ...
        $data = [
            ...
            'contact' => parseJsonField($contactRaw, 'Contact'),
            'social' => parseJsonField($socialRaw, 'Social')
        ];
        ...
        $notice = 'Aloqa kontenti saqlandi.';
...
<title>Aloqa — CHIMYON SCHOOL Admin</title>
<style>
...
<main>
<section class="intro"><div><p class="eyebrow">09 / CONNECTION</p><h1>Aloqa.</h1></div><p>CHIMYON SCHOOL bilan bog‘lanish uchun rasmiy aloqa va ijtimoiy tarmoq ma’lumotlarini boshqaring.</p></section>
...
<div class="guide"><strong>Contact data</strong><p>Telefon, email, manzil va boshqa mavjud kontakt fieldlarini JSON object/array sifatida saqlang. Uydirma kontakt ma’lumotlari kiritmang.</p></div>
<div class="field wide"><label for="contact">Contact JSON</label><textarea id="contact" name="contact" rows="12" spellcheck="false" placeholder='{"phone":"...","email":"...","address":"..."}'><?=htmlspecialchars(jsonText($data['contact']),ENT_QUOTES,'UTF-8')?></textarea></div>
<div class="guide"><strong>Social links</strong><p>Faqat rasmiy va tasdiqlangan ijtimoiy tarmoq URL’larini kiriting.</p></div>
<div class="field wide"><label for="social">Social JSON</label><textarea id="social" name="social" rows="10" spellcheck="false" placeholder='{"instagram":"...","telegram":"...","facebook":"..."}'><?=htmlspecialchars(jsonText($data['social']),ENT_QUOTES,'UTF-8')?></textarea><small>Valid JSON object/array. Bo‘sh qoldirish mumkin.</small></div>
</div><div class="actions"><div class="hint">JSON UTF-8 · pretty-print · file locking · zero dependencies.</div><button class="save" type="submit">SAVE CHANGES →</button></div></form>
<p class="footer">data/aloqa.json · PHP 8.x · current contact/social fields preserved.</p>
</main></div></body></html>
