...
$file = $root . '/data/yangiliklar.json';
$notice = '';
$error = '';
$default = ['intro' => '', 'hero_image' => '', 'posts' => []];

function readNews(string $file, array $default): array {
...
function newsLinesToArray(string $value): array {
    $items = [];
    foreach (preg_split('/\R/', $value) ?: [] as $line) {
        $line = trim($line);
        if ($line === '') continue;
        $decoded = json_decode($line, true);
        if (!is_array($decoded)) throw new RuntimeException('Har bir post qatori JSON object bo‘lishi kerak.');
        $items[] = $decoded;
    }
    return $items;
}
function newsToLines(mixed $value): string {
...
$data = readNews($file, $default);
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $intro = trim((string)($_POST['intro'] ?? ''));
    $heroImage = trim((string)($_POST['hero_image'] ?? ''));
    $postsRaw = trim((string)($_POST['posts'] ?? ''));
    try {
        if ($heroImage !== '' && !preg_match('#^(?:https?://|/|\.\./|media/)#i', $heroImage)) throw new RuntimeException('Hero image path xavfsiz formatda emas.');
        $posts = $postsRaw === '' ? [] : newsLinesToArray($postsRaw);
        $data = ['intro' => $intro, 'hero_image' => $heroImage, 'posts' => $posts];
...
<header class="top"><div class="brand">CHIMYON / ADMIN</div><a class="back" href="index.php">← Control center</a></header><main>
<section class="intro"><div><p class="eyebrow">06 / EDITORIAL</p><h1>Yangiliklar.</h1></div><p>Yangiliklar va voqealarni mavjud JSON modelini saqlagan holda boshqaring. Faqat real, tekshirilgan kontentni e’lon qiling.</p></section>
...
<div class="field wide"><label for="hero_image">Hero image path</label><input id="hero_image" name="hero_image" value="<?= htmlspecialchars((string)$data['hero_image'], ENT_QUOTES, 'UTF-8') ?>" placeholder="media/images/..."><small>Faqat mavjud media path yoki ishonchli URL.</small></div>
<div class="guide"><strong>Post records</strong><p>Har bir qator bitta JSON object. Masalan: <code>{"title":"...","date":"...","image":"...","excerpt":"..."}</code>. Mavjud fieldlarni o‘zgartirmasdan saqlash tavsiya etiladi.</p></div>
<div class="field wide"><label for="posts">Posts</label><textarea id="posts" name="posts" rows="20" spellcheck="false" placeholder='{"title":"...","date":"...","image":"...","excerpt":"..."}'><?= htmlspecialchars(newsToLines($data['posts']), ENT_QUOTES, 'UTF-8') ?></textarea><small>Har bir qatorda alohida JSON object. Bo‘sh qoldirilsa posts array bo‘sh saqlanadi.</small></div>
...
<p class="footer">data/yangiliklar.json · PHP 8.x · zero dependencies · current posts: <?= count(is_array($data['posts']) ? $data['posts'] : []) ?></p>
</main></div></body></html>
